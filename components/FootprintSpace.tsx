...
const COUNTRIES_LIST = [
  "United States", "United Kingdom", "Japan", "South Korea", "France", "Germany", 
  "Italy", "Spain", "Australia", "Canada", "Singapore", "Thailand", "Vietnam", 
  "Malaysia", "Indonesia", "India", "Russia", "Brazil", "Argentina", "Mexico", 
  "Egypt", "South Africa", "Turkey", "United Arab Emirates", "Saudi Arabia",
  "Netherlands", "Switzerland", "Sweden", "Norway", "Finland", "Denmark",
  "New Zealand", "Philippines", "Austria", "Belgium", "Portugal", "Greece",
  "Ireland", "Poland", "Czech Republic", "Hungary", "Iceland"
];
...
export const FootprintSpace: React.FC<FootprintSpaceProps> = ({ theme }) => {
  ...
  const [stats, setStats] = useState<FootprintStats>({
    totalCount: 0, countries: [], provinces: [], citiesCount: 0
  });
  ...
  const [currentFootprint, setCurrentFootprint] = useState<Partial<Footprint>>({
    ...
    mood: 'happy',
    location: { name: '', coordinates: [116.40, 39.90] } // Default Beijing
  });
  ...
  const handleOpenAdd = () => {
     ...
     setCurrentFootprint({
        ...
        location: { 
            name: '', 
            coordinates: initialScope === 'CHINA' ? [116.40, 39.90] : [0, 20],
  ...
  const handlePickerMapInit = () => {
     if (pickerContainerRef.current) {
         ...
         const coords = currentFootprint.location?.coordinates || [116.40, 39.90];
    ...
    <div className="container mx-auto px-4 py-24 pt-32 max-w-7xl relative z-10 min-h-screen flex flex-col">
       ...
       <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
             <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">{t.footprint.stats.total}</div>
             <div className="text-2xl font-bold text-slate-800 dark:text-white">{stats.totalCount}</div>
          </div>
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
             <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">{t.footprint.stats.countries}</div>
             <div className="text-2xl font-bold text-slate-800 dark:text-white">{stats.countries.length}</div>
          </div>
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
             <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">{t.footprint.stats.provinces}</div>
             <div className="text-2xl font-bold text-primary-500">{stats.provinces.length}</div>
          </div>
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
             <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">{t.footprint.stats.cities}</div>
             <div className="text-2xl font-bold text-slate-800 dark:text-white">{stats.citiesCount}</div>
          </div>
       </div>
       ...
       <div className="w-full h-[80vh] bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden">
          ...
          {viewMode === 'WORLD' && (
             <div ref={leafletContainerRef} style={{ width: '100%', height: '100%', isolation: 'isolate', zIndex: 0 }}></div>
          )}
       ...
       <div className="mt-12">
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6">Recent Transmission Logs</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {footprints.map(fp => (
                <div key={fp._id} onClick={() => handleEdit(fp)} className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                   <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                         <span className={`w-2 h-2 rounded-full ${fp.status === 'visited' ? 'bg-emerald-500' : 'bg-blue-500'}`}></span>
                         <span className="text-xs font-bold uppercase text-slate-500">{fp.status}</span>
                      </div>
                      <span className="text-xs font-mono text-slate-400">{new Date(fp.visitDate).toLocaleDateString()}</span>
                   </div>
                   
                   <h4 className="font-bold text-lg text-slate-800 dark:text-white mb-1 group-hover:text-primary-500 transition-colors">{fp.location.name}</h4>
                   ...
                   {fp.images && fp.images.length > 0 && (
                      <div className="h-32 rounded-xl overflow-hidden mb-3 bg-slate-100 dark:bg-slate-800">
                         <img src={fp.images[0]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                      </div>
                   )}
                   
                   <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">{fp.content}</p>
                </div>
             ))}
          </div>
       </div>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
             <div className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="w-full md:w-1/2 h-64 md:h-auto relative bg-slate-100 dark:bg-slate-950 overflow-hidden md:rounded-l-3xl md:rounded-tr-none rounded-t-3xl">
                   ...
                   <div ref={pickerContainerRef} style={{ width: '100%', height: '100%', isolation: 'isolate', zIndex: 0 }}></div>
                ...
                <div className="w-full md:w-1/2 p-6 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900">
                   <div className="flex justify-between items-center mb-4">
                      ...
                      <button onClick={() => setIsModalOpen(false)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 flex items-center justify-center transition-colors">
                   ...
                   <form onSubmit={handleSave} className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.name}</label>
                            <input 
                               required
                               ...
                               onChange={e => setCurrentFootprint(prev => ({...prev, location: {...prev.location!, name: e.target.value}}))}
                            />
                         </div>
                         <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.date}</label>
                            <input 
                               type="date"
                               className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary-500 text-slate-900 dark:text-white"
                               value={currentFootprint.visitDate ? new Date(currentFootprint.visitDate).toISOString().split('T')[0] : ''}
                               onChange={e => setCurrentFootprint(prev => ({...prev, visitDate: e.target.value}))}
                            />
                         </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         <div>
                            {formScope === 'CHINA' ? (
                               <>
                                <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.province}</label>
                                <select 
                                   className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary-500 text-slate-900 dark:text-white"
                                   value={currentFootprint.location?.province || ''}
                                   onChange={e => setCurrentFootprint(prev => ({...prev, location: {...prev.location!, province: e.target.value, country: 'China'}}))}
                                >
                                   <option value="">Select Province</option>
                                   {PROVINCES_CN.map(p => <option key={p} value={p}>{p}</option>)}
                                </select>
                               </>
                            ) : (
                               <>
                                ...
                                <select 
                                   ...
                                   onChange={e => setCurrentFootprint(prev => ({...prev, location: {...prev.location!, country: e.target.value, province: ''}}))}
                                >
                         ...
                         <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.city}</label>
                            <input 
                               className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary-500 text-slate-900 dark:text-white"
                               value={currentFootprint.location?.city || ''}
                               onChange={e => setCurrentFootprint(prev => ({...prev, location: {...prev.location!, city: e.target.value}}))}
                            />
                         </div>
                      </div>
                      ...
                      <div>
                         <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Coordinates</label>
                         <div className="text-xs font-mono text-slate-500 bg-slate-50 dark:bg-slate-800 p-2 rounded border border-slate-200 dark:border-slate-700">
                            {currentFootprint.location?.coordinates?.[0].toFixed(6)}, {currentFootprint.location?.coordinates?.[1].toFixed(6)}
                         </div>
                      </div>

                      {/* Content & Mood */}
                      <div>
                         <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.content}</label>
                         <textarea 
                            ...
                            onChange={e => setCurrentFootprint(prev => ({...prev, content: e.target.value}))}
                         />
                      </div>

                      <div className="flex justify-between items-center">
                         <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.status}</label>
                            <div className="flex gap-2">
                               <button type="button" onClick={() => setCurrentFootprint(p => ({...p, status: 'visited'}))} className={`px-3 py-1 text-xs rounded-full border transition-all ${currentFootprint.status === 'visited' ? 'bg-emerald-500 text-white border-emerald-500' : 'text-slate-500 border-slate-300 dark:border-slate-600'}`}>{t.footprint.form.visited}</button>
                               <button type="button" onClick={() => setCurrentFootprint(p => ({...p, status: 'planned'}))} className={`px-3 py-1 text-xs rounded-full border transition-all ${currentFootprint.status === 'planned' ? 'bg-blue-500 text-white border-blue-500' : 'text-slate-500 border-slate-300 dark:border-slate-600'}`}>{t.footprint.form.planned}</button>
                            </div>
                         </div>
                         
                         <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">{t.footprint.form.mood}</label>
                            <select 
                               ...
                               onChange={e => setCurrentFootprint(p => ({...p, mood: e.target.value as any}))}
                            >
                      ...
                      {/* Photos */}
                      <div>
                         <label className="block text-xs font-bold text-slate-400 uppercase mb-2">{t.footprint.form.photos}</label>
                         <div className="flex flex-wrap gap-2 mb-2">
                            {currentFootprint.images?.map((img, i) => (
                               <img key={i} src={img} className="w-16 h-16 object-cover rounded-lg border border-slate-200 dark:border-slate-700" />
                            ))}
                         </div>
                         <button 
                            type="button" 
                         ...
                         >
                            {isUploading ? <i className="fas fa-circle-notch fa-spin"></i> : <i className="fas fa-camera"></i>} Upload Photo
                         </button>
