...
export const SystemManagement: React.FC = () => {
  ...
  const confirmDeleteImage = async () => {
      ...
      try {
          ...
          // Revert or re-fetch logic could go here if strict
      }
  };
    ...
    return (
      <div className="container mx-auto px-6 py-32 text-center">
        <div className="inline-block p-6 rounded-full bg-slate-100 dark:bg-slate-800 mb-6">
           <i className="fas fa-server text-4xl text-slate-400"></i>
        </div>
        <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-300">System Data Unavailable</h2>
        <p className="text-slate-500 mt-2">Could not retrieve usage metrics from the mainframe.</p>
        <button onClick={fetchUsage} className="mt-6 px-6 py-2 bg-primary-500 text-white rounded-lg font-bold hover:bg-primary-600 transition-colors">
           Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-6 py-24 pt-32 max-w-6xl animate-fade-in relative z-10 min-h-screen">
      ...
      {/* Header */}
      <div className="mb-12 border-b border-slate-200 dark:border-slate-800 pb-8 flex justify-between items-end">
        <div>
        ...
        <div className="text-right hidden sm:block">
           <div className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">{t.system.cloudinary.lastUpdated}</div>
           <div className="font-mono text-sm text-slate-600 dark:text-slate-300">{new Date(usageData.last_updated).toLocaleDateString()}</div>
        </div>
      </div>
