...
export const ArticleView: React.FC<ArticleViewProps> = ({ onBack, onNavigateToBlog, currentUser, onLoginRequest }) => {
  ...
  const formatLocalTime = (dateStr?: string, detailed = true) => {
    if (!dateStr) return '';
    try {
  ...
  if (!blog && !isLoading) {
      return (
          <div className={containerClass}>
              ...
              <button onClick={onBack} className="mt-4 text-primary-500 block mx-auto hover:underline">Back to Journal</button>
          </div>
      )
  }
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedPosts.map(post => (
                ...
                >
                   ...
                   <div>
                      <span className="text-xs font-mono text-slate-400 uppercase tracking-wider block mb-2">{formatLocalTime(post.createdDate || post.date, false)}</span>
