...
export const RoleManagement: React.FC = () => {
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl animate-fade-in min-h-[600px]">
      ...
      {loading ? (
        <div className="text-center py-20 opacity-50">{t.system.common.loading}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {roleList.map((role) => (
            <div
              key={role._id}
              className="bg-white dark:bg-slate-950 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative group overflow-hidden hover:border-primary-500/30 transition-all"
            >
              <div className="flex justify-between items-start mb-2">
                <h4 className="font-bold text-lg text-slate-800 dark:text-white uppercase tracking-wider">
                  {role.name}
                </h4>
              ...
              <div className="text-xs text-slate-400 font-mono mb-1">
                {t.system.roles.perms} ({role.permissions.length}):
              </div>
              <div className="flex flex-wrap gap-1">
                {role.permissions.slice(0, 5).map((p) => (
                  <span
                    key={p}
                    className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 rounded text-[9px] font-mono border border-slate-200 dark:border-slate-800"
                  >
                    {p}
                  </span>
                ))}
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">
                  {editingRole._id ? t.system.roles.edit : t.system.roles.add}
                </h3>
                <button
                  onClick={() => setEditingRole(null)}
                  className="w-8 h-8 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                >
              ...
              <form onSubmit={handleSaveRole} className="flex-1 flex flex-col min-h-0">
                <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase text-slate-400 mb-2">
                      {t.system.roles.perms}
                    </label>
                    <div className="space-y-4">
                      {Object.entries(groupedPermissions).map(([category, perms]) => (
                        <div
                          key={category}
                          className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800"
                        >
                          <h5 className="text-xs font-bold uppercase text-primary-500 mb-2">
                            {category}
                          </h5>
                          <div className="grid grid-cols-2 gap-2">
                            {perms.map((p: Permission) => {
                              const currentPerms = editingRole.permissions || [];
                              const isChecked = currentPerms.some(
                                (cp) => cp.toUpperCase() === p.key.toUpperCase()
                              );

                              return (
                                <label
                                  key={p.key}
                                  className={`flex items-center gap-2 p-2 rounded-lg border transition-all cursor-pointer ${isChecked ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:border-slate-300'}`}
                                >
                                  <input
                                    type="checkbox"
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (checked) {
                                        ...
                                        newPerms = current.filter(
                                          (k) => k.toUpperCase() !== p.key.toUpperCase()
                                        );
                                      }
                                      setEditingRole((prev) =>
                                        prev ? { ...prev, permissions: newPerms } : null
                                      );
                                    }}
                                  />
                ...
                <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setEditingRole(null)}
                    className="px-4 py-2 rounded-lg text-sm font-bold text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800"
                  >
                    {t.system.common.cancel}
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2 rounded-lg text-sm font-bold bg-primary-600 text-white hover:bg-primary-700 shadow-lg"
                  >
                    {t.system.roles.save}
                  </button>
                </div>
