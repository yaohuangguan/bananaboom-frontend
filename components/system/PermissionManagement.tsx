  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl animate-fade-in min-h-[600px]">
      <div className="flex justify-between items-center mb-6 border-b border-slate-200 dark:border-slate-700 pb-2">
        ...
        <button
          onClick={() =>
            setEditingPerm({ key: '', name: '', description: '', category: 'General' })
          }
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            <thead className="text-xs uppercase text-slate-400 bg-slate-50 dark:bg-slate-800">
              <tr>
                ...
                <th className="px-4 py-3 rounded-tr-lg text-right">{t.system.common.actions}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {permList.map((perm) => (
                ...
                >
                  <td className="px-4 py-3 font-mono text-xs font-bold text-slate-800 dark:text-white">
                    {perm.key}
                  </td>
                  <td className="px-4 py-3">{perm.name}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] font-bold uppercase tracking-wider">
                      {perm.category}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    {perm.key !== '*' && (
                      <button
                        onClick={() => setPermToDelete(perm.key)}
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl flex flex-col overflow-hidden">
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">
                  {editingPerm._id ? t.system.permissions.edit : t.system.permissions.add}
                </h3>
                <button
              ...
              <form onSubmit={handleSavePerm} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingPerm({ ...editingPerm, key: e.target.value.toUpperCase() })
                    }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditingPerm({ ...editingPerm, description: e.target.value })
                    }
                  />
                </div>
      ...
      <DeleteModal
        ...
        title={t.system.permissions.delete}
