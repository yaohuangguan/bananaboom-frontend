  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl animate-fade-in min-h-[600px]">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requests.map((req) => (
            <div
              key={req._id}
              className="bg-white dark:bg-slate-950 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm relative group overflow-hidden"
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-200 shrink-0">
                  <img
                    src={
                      req.user?.photoURL ||
                      `https://ui-avatars.com/api/?name=${req.user?.displayName || 'U'}`
                    }
      ...
      <DeleteModal
        ...
        title={t.system.requests.reject}
        message="Are you sure you want to reject this permission request?"
        requireInput={false}
        buttonText={t.system.requests.reject}
      />
    </div>
  );
};
