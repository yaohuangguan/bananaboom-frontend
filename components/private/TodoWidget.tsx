...
export const TodoWidget: React.FC = () => {
  ...
  const handlePaste = async (e: React.ClipboardEvent) => {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i].type.indexOf('image') !== -1) {
        ...
        if (file) {
          e.preventDefault(); // Prevent pasting binary string into text area
          setIsProcessing(true);
          try {
            const url = await apiService.uploadImage(file);
            setCurrentTodo((prev) => ({
          ...
          return; // Only upload one at a time per paste
        }
      }
    }
  };
  ...
  return (
    <div
      className={`bg-pink-50/80 rounded-3xl shadow-lg border border-pink-200 transition-all duration-500 relative flex flex-col ${isExpanded ? 'h-[500px]' : 'h-16'} overflow-hidden group`}
    >
      {/* Header Bar */}
      <div
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 leading-none">
              {viewType === 'wish' ? t.privateSpace.bucketList.title : 'Daily Routine'}
            </h3>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 custom-scrollbar space-y-3">
          {isLoading ? (
            <div className="text-center py-10 text-pink-300 text-xs animate-pulse">
              Loading list...
            </div>
            ...
            filteredTodos.map((todo) => {
              const authorAvatar =
                todo.user?.photoURL ||
                `https://ui-avatars.com/api/?name=${encodeURIComponent(todo.user?.displayName || 'Anonymous')}&background=random`;
                      ...
                      className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${
                        itemStatus === 'done'
                          ...
                          : itemStatus === 'in_progress'
                            ? 'bg-white border-pink-400 text-pink-400 animate-pulse'
                    ...
                    {itemStatus === 'done' && (
                      <>
                        ...
                        >
                          ...
                          {t.privateSpace.bucketList.actions.wishlist}
                        </button>
                        <button
                          onClick={(e) => handleQuickStatusUpdate(e, todo, 'in_progress')}
                          className="flex-1 py-1.5 rounded-lg bg-blue-50 text-blue-500 text-[10px] font-bold uppercase hover:bg-blue-100 transition-colors flex items-center justify-center gap-1"
                        ...
                        >
                          ...
                          {t.privateSpace.bucketList.actions.restart}
                        </button>
                      </>
                    )}
                  </div>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <input
                  ...
                  placeholder={t.privateSpace.bucketList.placeholder + ' (Item, Goal...)'}
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      value={
                        currentTodo.targetDate
                          ? new Date(currentTodo.targetDate).toISOString().split('T')[0]
                          : ''
                      }
                      onChange={(e) =>
                        setCurrentTodo((prev) => ({ ...prev, targetDate: e.target.value }))
                      }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setCurrentTodo((prev) => ({ ...prev, description: e.target.value }))
                    }
                    // Explicit text color for visibility
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    >
                      ...
                      <span className="text-[9px] opacity-60 ml-1 font-normal hidden sm:inline">
                        (or Paste Ctrl+V)
                      </span>
                    </button>
                    <input
