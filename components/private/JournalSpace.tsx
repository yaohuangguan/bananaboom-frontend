                  ...
                  <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar bg-white">
                      <div className="max-w-2xl mx-auto">
                          <div className="mb-8 text-center">
                              <div className="flex justify-center gap-2 mb-4">
                                  {selectedEntry.tags.map(t => (
                                      <span key={t} className="px-3 py-1 bg-rose-50 text-rose-500 rounded-full text-[10px] font-bold uppercase tracking-widest">{t}</span>
                                  ))}
                              </div>
                          ...
                          <div className="border-t border-slate-100 pt-8">
                              <CommentsSection postId={selectedEntry._id} currentUser={user} onLoginRequest={() => {}} forceLight={true} />
                          </div>
                      </div>
                  </div>
              </div>
              ...
              <div className="flex-col h-full flex animate-fade-in bg-white/50">
                  <div className="p-6 pb-4 bg-amber-50/50 border-b border-amber-100 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-amber-200 flex items-center justify-center text-amber-700 shadow-sm animate-pulse"><i className="fas fa-eye"></i></div>
                        <h2 className="text-lg font-display font-bold text-slate-700 uppercase tracking-widest">Live Preview</h2>
                    </div>
                    <button onClick={() => setIsPreviewHidden(true)} className="w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-200 flex items-center justify-center transition-colors shadow-sm"><i className="fas fa-times"></i></button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-50/30">
                      ...
                      <BlogContent content={previewData?.content || ''} shadowClass="shadow-sm" forceLight={true} />
                  </div>
              </div>
          ) : (
              // List View
                              ...
                              <div className="min-w-0">
                                  ...
                                  <div className="flex items-center gap-2">
                                      ...
                                      {hasContent(previewData) && isPreviewHidden && (
                                          <button onClick={() => setIsPreviewHidden(false)} className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-600 text-[10px] font-bold uppercase rounded-full animate-pulse hover:bg-amber-200 transition-colors"><i className="fas fa-eye mr-1"></i> Resume Preview</button>
                                      )}
                                  </div>
                              </div>
                          </div>
                          <div className="flex bg-slate-100 p-1 rounded-lg shrink-0">
                              <button onClick={() => setLogSource('private')} className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${logSource === 'private' ? 'bg-white text-rose-500 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Private</button>
                              <button onClick={() => setLogSource('public')} className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${logSource === 'public' ? 'bg-white text-blue-500 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Public</button>
                          </div>
                      </div>
                      <div className="relative group">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><i className={`fas fa-search transition-colors ${logSource === 'private' ? 'text-rose-300 group-focus-within:text-rose-500' : 'text-blue-300 group-focus-within:text-blue-500'}`}></i></div>
                          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search logs..." className={`block w-full pl-10 pr-3 py-2 bg-white/80 border rounded-xl leading-5 placeholder-slate-300 text-slate-700 focus:outline-none focus:ring-2 sm:text-sm transition-all shadow-sm ${logSource === 'private' ? 'border-rose-100 focus:ring-rose-400/50 focus:border-rose-400' : 'border-blue-100 focus:ring-blue-400/50 focus:border-blue-400'}`}/>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-50/30">
                      {isPublicLoading ? (
                          ...
                          <PrivateBlogFeed 
                              ...
                              onLike={handleLike} 
                              onEdit={(blog) => { setEditingPost(blog); if (window.innerWidth < 1024) setTimeout(() => document.getElementById('private-editor')?.scrollIntoView({ behavior: 'smooth' }), 100); }}
