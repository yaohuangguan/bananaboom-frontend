...
export const NewsWidget: React.FC = () => {
  ...
  const fetchData = async (force = false) => {
    ...
    try {
      ...
      const timeStr = res.date || new Date().toISOString(); 
  ...
  const getRankIcon = (index: number) => {
    switch (index) {
      case 0: return <i className="fas fa-trophy text-yellow-500"></i>;
      case 1: return <i className="fas fa-medal text-slate-400"></i>;
      case 2: return <i className="fas fa-medal text-amber-700"></i>;
      default: return <span className="font-mono text-slate-300 w-4 text-center">{index + 1}</span>;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 bg-gradient-to-r from-blue-50 to-white border-b border-blue-100 p-4 relative">
        <div className="flex justify-between items-center mb-3">
           <div className="flex items-center gap-2 pointer-events-none">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-400 to-cyan-500 flex items-center justify-center text-white shadow-md">
                 <i className={`fas ${activeTab === 'hotsearch' ? 'fa-fire' : activeTab === 'finance' ? 'fa-chart-line' : activeTab === 'game' ? 'fa-gamepad' : 'fa-globe'}`}></i>
              </div>
              <div>
                 <h3 className="text-sm font-bold text-slate-800 leading-none">{t.privateSpace.hotSearch.title}</h3>
                 <p className="text-[9px] text-blue-400 font-mono mt-0.5">{lastUpdate ? `${t.privateSpace.hotSearch.updated} ${lastUpdate}` : t.privateSpace.hotSearch.loading}</p>
              </div>
           </div>
           <div className="flex gap-2 no-drag">
              <button onClick={() => fetchData(true)} className="w-8 h-8 rounded-full bg-white text-slate-400 hover:text-blue-500 hover:bg-blue-50 flex items-center justify-center transition-colors shadow-sm border border-slate-100">
                 <i className={`fas fa-sync-alt text-xs ${loading ? 'fa-spin' : ''}`}></i>
              </button>
              <button onClick={() => setIsOpen(false)} className="w-8 h-8 rounded-full bg-blue-500 text-white hover:bg-blue-600 flex items-center justify-center transition-colors shadow-md shadow-blue-200">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl no-drag overflow-x-auto gap-1" onPointerDown={e => e.stopPropagation()}>
           <button 
      ...
      {/* List Content */}
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 bg-white/50 cursor-default" onPointerDown={e => e.stopPropagation()}>
         {loading && currentList.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-blue-300 gap-2">
               <i className="fas fa-circle-notch fa-spin text-2xl"></i>
               <span className="text-xs uppercase tracking-widest">{t.privateSpace.hotSearch.loading}</span>
            </div>
         ) : (
               ...
               {currentList.map((item, index) => (
                  ...
                  >
                     ...
                     <div className="flex-1 min-w-0">
                        <div className={`text-xs font-medium truncate ${index < 3 ? 'text-slate-800 font-bold' : 'text-slate-600'}`}>
                           {item.title}
                        </div>
                        {item.hot && (
