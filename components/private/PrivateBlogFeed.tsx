  ...
  return (
    <div className="space-y-6 pb-20">
      {blogs.map((blog) => {
        ...
        const authorAvatar = blog.user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(blog.author || 'Anonymous')}&background=random`;
               ...
               <div className="flex flex-col gap-1">
                  ...
                  {blog.user && (
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-xs font-bold text-slate-600">{blog.user.displayName}</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-1.5">
                     {!blog.user && (
                        <div className="w-6 h-6 rounded-full overflow-hidden bg-slate-100 ring-1 ring-slate-100">
                           <img src={authorAvatar} alt={blog.author} className="w-full h-full object-cover" />
                        </div>
                     )}
