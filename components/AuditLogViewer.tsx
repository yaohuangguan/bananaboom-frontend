      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
         ...
         <div className="flex flex-col items-end gap-2">
            {(filterOperator || filterAction || targetSearch) && (
                ...
                {filterOperator && (
                    <button onClick={() => setFilterOperator(null)} className="px-3 py-1 rounded bg-slate-800 border border-slate-700 text-white text-xs hover:bg-slate-700 flex items-center gap-2">
                        <i className="fas fa-user text-cyan-400"></i> {filterOperator.displayName} <i className="fas fa-times opacity-50"></i>
                    </button>
                )}
                {filterAction && (
                    <button onClick={() => setFilterAction(null)} className="px-3 py-1 rounded bg-slate-800 border border-slate-700 text-white text-xs hover:bg-slate-700 flex items-center gap-2">
                        <i className="fas fa-bolt text-amber-400"></i> {filterAction} <i className="fas fa-times opacity-50"></i>
                    </button>
                )}
                {targetSearch && (
                    <button onClick={() => setTargetSearch('')} className="px-3 py-1 rounded bg-slate-800 border border-slate-700 text-white text-xs hover:bg-slate-700 flex items-center gap-2">
                        <i className="fas fa-search text-purple-400"></i> "{targetSearch}" <i className="fas fa-times opacity-50"></i>
                    </button>
                )}
                <button onClick={clearFilters} className="text-xs text-red-400 hover:text-red-300 underline ml-2">Clear All</button>
                </div>
            )}
            ...
            <div className="bg-[#050914] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl relative min-h-[600px] flex flex-col">
                           ...
                           {loading ? (
                              ...
                              logs.map((log) => (
                                 <tr key={log._id} className="hover:bg-cyan-900/10 transition-colors group">
                                       ...
                                       {log.operator ? (
                                          <div className="flex items-center gap-3">
                                             <div className="w-6 h-6 rounded-full overflow-hidden bg-slate-700 ring-1 ring-slate-600">
                                                <img src={log.operator.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(log.operator.displayName)}&background=random`} className="w-full h-full object-cover" />
                                             </div>
                                             <button 
                                                onClick={() => { setFilterOperator(log.operator as User); setViewMode('TIMELINE'); }} 
                                    ...
                                    <td className="p-4">
                                       <button 
                                          onClick={() => { setFilterAction(log.action); setViewMode('TIMELINE'); }} 
               ...
               {viewMode === 'OPERATORS' && (
                  <div className="flex-1 flex flex-col">
                     <div className="p-8">
                        ...
                        {operatorLoading ? (
                           <div className="text-center py-10 text-slate-500"><i className="fas fa-circle-notch fa-spin mr-2"></i> Loading users...</div>
                        ) : (
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              {operatorList.map(user => (
                                 <button 
                                    key={user._id}
                                    onClick={() => { setFilterOperator(user); setViewMode('TIMELINE'); }}
                                    className={`bg-slate-900 border p-4 rounded-xl flex items-center gap-4 transition-all hover:bg-slate-800 group text-left ${
                                       filterOperator?._id === user._id ? 'border-cyan-500/50 bg-cyan-900/10' : 'border-slate-700 hover:border-cyan-500/30'
                                    }`}
                                 >
                                    <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-500 overflow-hidden ring-2 ring-transparent group-hover:ring-cyan-500/30">
                                       <img src={user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName)}&background=random`} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="min-w-0">
                                       <div className="font-bold text-white group-hover:text-cyan-400 truncate">{user.displayName}</div>
                     ...
                     {operatorPagination && operatorPagination.totalPages > 1 && (
                        <div className="flex items-center justify-center gap-4 p-4 mt-auto border-t border-slate-800">
                           <button onClick={() => fetchOperators(operatorPagination.currentPage - 1)} disabled={!operatorPagination.hasPrevPage} className="text-slate-400 hover:text-white disabled:opacity-30"><i className="fas fa-chevron-left"></i></button>
                           <span className="text-xs text-slate-500 font-mono">Page {operatorPagination.currentPage} of {operatorPagination.totalPages}</span>
                           <button onClick={() => fetchOperators(operatorPagination.currentPage + 1)} disabled={!operatorPagination.hasNextPage} className="text-slate-400 hover:text-white disabled:opacity-30"><i className="fas fa-chevron-right"></i></button>
                        </div>
                     )}
                  </div>
               )}
               ...
               {viewMode === 'ACTIONS' && (
                  <div className="p-8 flex flex-col h-full">
                     ...
                     <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Recent / Known Commands</h4>
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-4 overflow-y-auto max-h-[400px] custom-scrollbar pr-2">
                        {Array.from(uniqueActions).length === 0 && <p className="text-slate-500 col-span-4 text-center">No actions found in current fetched logs.</p>}
                        {Array.from(uniqueActions).map(act => (
                           <button 
                              key={act}
                              onClick={() => { setFilterAction(act); setViewMode('TIMELINE'); }}
                              className={`bg-slate-900 border p-4 rounded-xl text-center transition-all hover:bg-slate-800 group flex flex-col items-center justify-center gap-3 aspect-video ${
                                 filterAction === act ? 'border-amber-500/50 bg-amber-900/10' : 'border-slate-700 hover:border-amber-500/50'
                              }`}
                           >
                              <i className="fas fa-terminal text-2xl text-slate-600 group-hover:text-amber-400"></i>
                              <div className="font-mono font-bold text-xs text-slate-300 group-hover:text-white break-all">{act}</div>
                           </button>
                        ))}
                     </div>
                  </div>
